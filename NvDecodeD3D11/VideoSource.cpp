...
VideoSource::VideoSource(const std::string sFileName, FrameQueue *pFrameQueue): hVideoSource_(0)
{
	...
	{
		printf("> cuvidCreateVideoSource() returned an error result = %08x\n", oResult);
		if (oResult == CUDA_ERROR_INVALID_SOURCE)
		{
			printf("> Video codec format is unsupported (CUDA_ERROR_INVALID_SOURCE) <\n");
		}
		throw (oResult);
	}
}
